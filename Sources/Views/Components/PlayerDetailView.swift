                        ...
                        VStack(spacing: 0) {
                            HStack(spacing: 6) {
                                ForEach(0..<20) { index in
                                    LargeVisualizerBar(isPlaying: playerManager.playbackState == .playing)
                                }
                            }
                            .frame(height: 160)
                            .padding(.top, 20)
                            ...
                            ZStack {
                                Circle()
                                    .stroke(Color.cyan.opacity(playerManager.playbackState == .playing ? 0.3 : 0.1), lineWidth: 2)
                                    .scaleEffect(playerManager.playbackState == .playing ? 1.25 : 1.0)
                                    .frame(width: 70, height: 70)
                                    .blur(radius: 2)
                                    .animation(playerManager.playbackState == .playing ? Animation.easeInOut(duration: 0.85).repeatForever(autoreverses: true) : .default, value: playerManager.playbackState)
...
/// Крупная анимированная полоса для режима Визуализатора
struct LargeVisualizerBar: View {
    let isPlaying: Bool
    @State private var heightMultiplier: CGFloat = 0.15
    ...
    var body: some View {
        RoundedRectangle(cornerRadius: 3.5)
            ...
            .frame(width: 8, height: 160 * heightMultiplier)
            .animation(
                isPlaying ?
                    Animation.easeInOut(duration: Double.random(in: 0.15...0.45))
            ...
            .onAppear {
                if isPlaying {
                    heightMultiplier = CGFloat.random(in: 0.2...1.0)
                }
            }
